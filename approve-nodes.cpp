...
#define OUTPUT_POINTS "output-checked.points"
...
#define NUM_NODES 689

//pixel locations of each node
point nodes[689] = {
    { 6648, 8880 },
    { 5581, 8870 },
    { 5285, 8866 },
...
{
    point p = nodes[current];

    int x = p.x - (WINDOW_X / 2);
    int y = p.y - (WINDOW_Y / 2);
    ...
    pa.x = 0;
    pb.x = WINDOW_X;
    pb.y = pa.y = (WINDOW_Y / 2);
    // line(cropped, pa, pb, color);

    //vertical
    pa.y = 0;
    pb.y = WINDOW_X;
    pb.x = pa.x = (WINDOW_X / 2);
    // line(cropped, pa, pb, color);
...
int main()
{
    ...
    while((k != 'q') && (current < NUM_NODES))
    {
        ...
        if(k != -1)
        {
            f << nodes[current].x << ", " << nodes[current].y << endl;
        }
